...
def lambda_handler(event, context):
    ...
    my_bucket = s3.Bucket('cduranmartinez')
    ...
    for file in my_bucket.objects.all():
        if file.last_modified.replace(tzinfo=None) == last_modified_date:
            ...
            archivo =file.key
            print(archivo)
            url = f"https://{'cduranmartinez'}.s3.us-east-1.amazonaws.com/{archivo}"
            print(url)
    ...
    s3.put_object(Bucket='qrdiplomas', Key=filename, Body=img_bytes, ContentType='image/png', ACL='public-read')
    
    # Generate the URL of the uploaded QR code
    location = s3.get_bucket_location(Bucket='qrdiplomas')['LocationConstraint']
    region = '' if location is None else f'{location}'
    qr_code_url = f"https://{'qrdiplomas'}.s3{region}.amazonaws.com/{filename}"
